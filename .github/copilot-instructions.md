...
**Commit 3 (done):** AutoCAD-thread marshaling. `AutoCadThreadDispatcher` in `Threading/` (lifted from Suite's `SuiteCadPipeHost.InvokeOnApplicationThread` pattern) captures the application thread id during `Initialize`, attaches `Application.Idle` handler, queues callbacks from background threads (Kestrel) and runs them on the AutoCAD UI thread on the next idle tick. Async API: `Task` / `Task<T> InvokeOnApplicationThreadAsync(...)`. First database-reading tool: `chamber19_get_active_document` returning `{name, path, isModified, ts}` — proves the dispatcher is load-bearing (a direct database read on the request thread faults; via the dispatcher it succeeds). Lifecycle ordering: register idle handler AFTER the host starts, deregister BEFORE the host stops.

**Slice 4+ (next):** introduce `fixtures/` with sample DWGs and `expected.json` per fixture as the eval discipline for read-only tools. Add more read-only tools (`list_layers`, `count_blocks_by_name`, `polyline_length_by_layer`, etc.) per the patterns under `autocad-knowledge/patterns/`. Plus the open issues from earlier slices: ALC isolation (#3) before any external distribution, serverInfo polish (#4) when convenient.
