...
**Slice 6+ (next):** more read-only tools per `autocad-knowledge/patterns/` — `count_blocks_by_name`, `polyline_length_by_layer`, `text_enumeration_by_layer`. Also: `fixtures/` with sample DWGs for end-to-end eval tests (the integration tests that the unit harness here doesn't cover). Open issues from earlier slices remain: ALC isolation (#3) before any external distribution, serverInfo polish (#4) when convenient.
