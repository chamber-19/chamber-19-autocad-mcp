...
**Slice 5+ (next):** introduce `fixtures/` with sample DWGs and `expected.json` per fixture as the eval discipline for AutoCAD-touching tools (the integration tests that the unit harness here doesn't cover). Add more read-only tools (`list_layers`, `count_blocks_by_name`, `polyline_length_by_layer`, etc.) per the patterns under `autocad-knowledge/patterns/`. Plus the open issues from earlier slices: ALC isolation (#3) before any external distribution, serverInfo polish (#4) when convenient.
